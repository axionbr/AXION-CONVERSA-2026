...
exports.continueExecutionWithResponse = continueExecutionWithResponse;
...
async function executeFlow(flowId, conversationId, leadId, testMode = false, forceRun = false) {
    const flow = await prisma.flow.findUnique({
        where: { id: flowId },
        include: { nodes: true, edges: true },
    });
    if (!flow || (!flow.active && !forceRun))
        return null;
...
async function continueExecutionWithResponse(executionId, userMessage) {
    const execution = await prisma.flowExecution.findUnique({
        where: { id: executionId },
        include: { flow: { include: { nodes: true, edges: true } } },
    });
    if (!execution || execution.status !== 'WAITING_RESPONSE')
        return;
    if (!execution.conversationId || !execution.currentNodeId)
        return;
    const currentNode = execution.flow.nodes.find((n) => n.id === execution.currentNodeId);
    await prisma.flowExecution.update({
        where: { id: executionId },
        data: { status: 'RUNNING' },
    });
    ...
    try {
        await executeNode(executionId, targetNodeId, execution.flow, execution.conversationId, execution.leadId ?? undefined, 0, execution.testMode ?? false, userMessage);
...
async function sendFlowMessage(conversationId, nodeId, text, senderType, testMode) {
    ...
    if (!testMode) {
        ...
        try {
            const result = await (0, zapiService_1.sendTextMessage)(zapiPhone, text.trim(), conversation.storeId);
        ...
        console.log(`[FLOW_SEND_SKIPPED_TESTMODE] | conv: ${conversationId} | node: ${nodeId} | msg salva no DB — Z-API não chamada`);
    }
...
async function runAgentNode(agentType, conversationId, nodeId, leadId, testMode) {
...
// ─── Executor de nó ───────────────────────────────────────────────────────────
async function executeNode(executionId, nodeId, flow, conversationId, leadId, depth = 0, testMode = false, userInput) {
    if (depth > 50)
        throw new Error('Flow loop detected — mais de 50 nós percorridos');
    ...
    try {
        ...
        switch (node.type) {
                ...
                if (!userInput) {
                    ...
                    if (menuText.trim()) {
                        ...
                        if (options.length > 0) {
                            ...
                            if (!hasNumberedLine) {
                                fullMenu += '\n\n' + options
                                    .map((opt, i) => `${opt.value ?? (i + 1)} - ${opt.label}`)
                                    .join('\n');
                            }
                        }
                    ...
                    for (let i = 0; i < options.length; i++) {
                        ...
                        const aliases = Array.isArray(opt.aliases)
                            ? opt.aliases.map((a) => a.toLowerCase())
                            : [];
                        if (normalized === optValue ||
                            normalized === optLabel ||
                    ...
                    if (matchedOption) {
                        ...
                        const matchedEdge = flow.edges.find((e) => {
                            ...
                            return (cond === String(matchedOption.value ?? '') ||
            ...
            case 'DELAY': {
                ...
                const unitMs = config.unit === 'hours' ? 3600000
...
function evaluateCondition(rule, lead, userInput) {
