...
async function processZapiWebhook(payload) {
    try {
        ...
        (0, socket_1.emitNewMessage)(conversation.id, messagePayload);
        ...
        if (!freshConv?.aiEnabled || freshConv.mode === 'HUMANO')
            return;
        ...
        const aiReply = await (0, aiService_1.generateAiResponse)(conversation.id, chatHistory, conversation.storeId);
        if (!aiReply)
            return;
        ...
        await (0, zapiService_1.sendTextMessage)(normalizedPhone, aiReply, conversation.storeId).catch(err => console.error('Z-API send error:', err.message));
