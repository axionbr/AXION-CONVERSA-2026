...
async function processZapiWebhook(payload) {
    ...
    try {
        ...
        const tempEscalated = (classification.temperature === 'QUENTE' || classification.temperature === 'URGENTE') &&
        ...
        if (prevTemperature !== classification.temperature) {
            ...
            (0, flowEngine_1.triggerFlowsByEvent)('TEMPERATURE_CHANGED', classification.temperature, conversation.id, lead.id)
                .catch((e) => console.error(`[FLOW] TEMPERATURE_CHANGED error:`, e.message));
        }
        ...
        if (tempEscalated) {
            ...
            (0, flowEngine_1.triggerFlowsByEvent)('LEAD_HOT', classification.temperature, conversation.id, lead.id)
                .catch((e) => console.error(`[FLOW] LEAD_HOT error:`, e.message));
...
async function extractAndSaveLeadData(conversationId, storeId, leadId, messages) {
    try {
        const analysis = await (0, aiService_1.analyzeConversation)(messages, storeId);
        const lead = await prisma.lead.findUnique({ where: { id: leadId } });
        if (!lead)
            return;
        const leadUpdate = {};
        // Nome: atualizar se ainda é o telefone ou se a IA identificou nome real
        if (analysis.nomeCliente &&
