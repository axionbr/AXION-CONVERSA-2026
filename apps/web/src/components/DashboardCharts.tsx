...
import { BarChart2, PieChart as PieIcon, TrendingUp, ArrowDownUp, Users, KanbanSquare } from 'lucide-react';
...
function EmptyChart({ msg = 'Sem dados para o período' }: { msg?: string }) {
  return (
...
export default function DashboardCharts({ period }: { period: string }) {
  ...
  const userLeadData = (data?.leadsByUser ?? []).map((d: any) => ({
    ...d,
    fill: '#F97316',
  }));

  // Funil Kanban
  const kanbanData = (data?.kanbanStages ?? []).map((d: any) => ({
    ...d,
    label: KANBAN_LABELS[d.stage] || d.stage,
    fill: KANBAN_COLORS[d.stage] || '#6B7280',
  }));
  ...
  const hasUserData   = userLeadData.some((d: any) => d.count > 0);
  const hasKanbanData = kanbanData.some((d: any) => d.count > 0);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div className={CHART_CARD}>
        ...
        {isLoading ? <SkeletonChart /> : !hasConvData ? <EmptyChart /> : (
          <ResponsiveContainer width="100%" height={200}>
            {period === 'today' ? (
              ...
              <LineChart data={convData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                ...
                <YAxis allowDecimals={false} tick={TICK_STYLE} />
                <Tooltip content={<CustomTooltip />} />
                <Line type="monotone" dataKey="count" name="Conversas" stroke="#F97316"
                  strokeWidth={2} dot={{ fill: '#F97316', r: 3 }} activeDot={{ r: 5 }} />
              </LineChart>
            )}
          </ResponsiveContainer>
        )}
      ...
      <div className={CHART_CARD}>
        ...
        {isLoading ? <SkeletonChart /> : !hasStatusData ? <EmptyChart /> : (
          <ResponsiveContainer width="100%" height={200}>
            <PieChart>
      ...
      <div className={CHART_CARD}>
        ...
        {isLoading ? <SkeletonChart /> : !hasTempData ? <EmptyChart /> : (
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={tempData} layout="vertical"
      ...
      <div className={CHART_CARD}>
        ...
        {isLoading ? <SkeletonChart /> : !hasMsgData ? <EmptyChart /> : (
          <ResponsiveContainer width="100%" height={200}>
      ...
      <div className={CHART_CARD}>
        ...
        {isLoading ? <SkeletonChart /> : !hasUserData ? (
          <EmptyChart msg="Nenhum lead com responsável atribuído" />
        ) : (
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={userLeadData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#343434" />
              <XAxis dataKey="name" tick={{ ...TICK_STYLE, fontSize: 10 }} interval={0}
                angle={userLeadData.length > 4 ? -30 : 0}
                textAnchor={userLeadData.length > 4 ? 'end' : 'middle'}
                height={userLeadData.length > 4 ? 48 : 24} />
              <YAxis allowDecimals={false} tick={TICK_STYLE} />
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="count" name="Leads" fill="#F97316" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
      ...
      <div className={CHART_CARD}>
        <p className={CHART_TITLE}>
          <KanbanSquare className="w-4 h-4 text-[#F97316]" />
          Funil Kanban
        </p>
        {isLoading ? <SkeletonChart /> : !hasKanbanData ? (
          <EmptyChart msg="Nenhum lead no Kanban ainda" />
        ) : (
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={kanbanData} layout="vertical"
              ...
              <CartesianGrid strokeDasharray="3 3" stroke="#343434" horizontal={false} />
              <XAxis type="number" allowDecimals={false} tick={TICK_STYLE} />
              <YAxis type="category" dataKey="label" tick={{ ...TICK_STYLE, fontSize: 10 }} width={80} />
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="count" name="Leads" radius={[0, 4, 4, 0]}>
                {kanbanData.map((entry: any, i: number) => (
                  <Cell key={i} fill={entry.fill} />
                ))}
              </Bar>
            </BarChart>
